...
def train_epoch_bayesian(data, loader, model, optim):
    ...
    epoch_comb_r_squared = stats.linregress(all_mean_preds, all_targets).rvalue**2
    preds = np.array(all_mean_preds)
    targets = np.array(all_targets)
    epoch_mse = np.mean(np.square(preds - targets))


    summary_dict = {
        "loss_mean": epoch_loss / num_batches,
        "comb_r_squared": epoch_comb_r_squared,
        "kl_loss": epoch_mse+kl_weight*kl.item()
    }
...
def test_epoch(data, loader, model):
    ...
    with torch.no_grad():
        for _, drug_drug_batch in enumerate(loader):
            ...
            drug_batch = drug_drug_batch[0].tolist()
            #condatinate all batches to get the entire dataset 
            drugs = drugs + drug_batch
...
#BasicTrainer modified for Bayesian
class BayesianBasicTrainer(tune.Trainable):
    def setup(self, config):
        ...
        # Added a Test loader since we have train/validation/test split
    ...
    def step(self):
        ...
        if((self.patience >= self.patience_stop) | (self.training_it > self.iterations_stop)):
            results = []
            r_squared = []
            spearman = []
            for i in range(10): #define the number of realizations 
                ...
                r_squared.append(test_metrics['comb_r_squared'])
                spearman.append(test_metrics['spearman'])
                test_metrics = [(f"test {i}/" + k, v) for k, v in test_metrics.items()]
                metrics.update(dict(test_metrics))
                
            mean_values = [sum(col) / len(results) for col in zip(*results)]
            std_dev_values = [((sum((x - mean) ** 2 for x in col) / len(results)) ** 0.5) for col, mean in zip(zip(*results), mean_values)]
            metrics['mean_r_squared'] = sum(r_squared) / len(r_squared)
            metrics['std_r_squared'] = np.std(r_squared)
            metrics['mean_spearman'] = sum(spearman) / len(spearman)
            metrics['std_spearman'] = np.std(spearman)
            #metrics['std_dev'] = list(std_dev_values)
            #metrics['means'] = list(mean_values)
            #metrics['drugs'] = list(drugs)
...
class BayesianActiveTrainer(BayesianBasicTrainer):
    ...
    def step(self):
        ...
        for i in range(10):
            ...
            spearman.append(unseen_metrics['spearman'])
            #unseen_metrics = [(f"eval {i}/" + k, v) for k, v in unseen_metrics.items()]
          
        mean_values = [sum(col) / len(results) for col in zip(*results)]
        std_dev_values = [((sum((x - mean) ** 2 for x in col) / len(results)) ** 0.5) for col, mean in zip(zip(*results), mean_values)]
        unseen_metrics['mean_r_squared'] = sum(r_squared) / len(r_squared)
        unseen_metrics['std_r_squared'] = np.std(r_squared)
        unseen_metrics['std_spearman'] = np.std(spearman)
        unseen_metrics['mean_spearman'] = sum(spearman) / len(spearman)
        #unseen_metrics['std_dev'] = list(std_dev_values)
        #unseen_metrics['means'] = list(mean_values)
        #unseen_metrics['drugs'] = list(drugs)
        unseen_metrics.update(dict(unseen_metrics))
        
        #transposing the results and converting to tensor to make it compatible with original code
        unseen_preds = list(map(list, zip(*results)))
        unseen_preds = torch.tensor(unseen_preds)
